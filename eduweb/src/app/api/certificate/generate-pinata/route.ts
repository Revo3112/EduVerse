 ...
 *
 * Generates a personalized course completion certificate and uploads to Pinata PUBLIC IPFS
 * Compatible with CertificateManager.sol (ERC-1155) blockchain structure
 *
 * IMPORTANT: Certificate images are uploaded as PUBLIC for MetaMask/OpenSea compatibility
 * Private uploads use signed URLs that expire, breaking NFT metadata standards
 *
 * Request Body:
 * {
 *   // === REQUIRED FIELDS ===
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const certificateId = `cert-${body.courseId}-${Date.now()}-${Math.random()
      .toString(36)
      .substring(2, 9)}`;
    ...
    console.log(
      "[Certificate Generation API] Upload type: PUBLIC IPFS (MetaMask compatible)"
    );
    if (certificateData.tokenId) {
      console.log(
        "[Certificate Generation API] Blockchain Token ID:",
    ...
    if (!result.success) {
      console.error(
        "[Certificate Generation API] Certificate generation failed:",
        result.error
      );
      return NextResponse.json(
        {
    ...
    console.log(
      "[Certificate Generation API] Certificate generated successfully"
    );
...
export async function GET() {
  return NextResponse.json({
    status: "ok",
    service: "Pinata Certificate Generation API (Public IPFS)",
    version: "2.0.0",
    uploadType: "public",
