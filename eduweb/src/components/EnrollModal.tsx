...
import { Award, BookOpen, Clock, X, Zap, Loader2 } from "lucide-react";
...
interface EnrollModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onEnroll: (courseId: bigint, duration: number) => Promise<void>;
}
...
const PriceDisplay = memo<{ ethAmount: string; className?: string }>(
  ({ ethAmount, className = "" }) => {
...
const EnrollModal = memo<EnrollModalProps>(
  ({ course, isOpen, onClose, onEnroll }) => {
    ...
    const selectedOption = useMemo(
      () =>
        DURATION_OPTIONS.find((option) => option.months === selectedDuration) ||
        DURATION_OPTIONS[1],
      [selectedDuration]
    );
    ...
    const handleEnroll = useCallback(async () => {
      setIsEnrolling(true);
      try {
        await onEnroll(course.id, selectedDuration);
      } catch (error) {
        console.error("[EnrollModal] Enrollment failed:", error);
      } finally {
    ...
    const savingsText =
      selectedOption.discount > 0 ? `Save ${selectedOption.discount}%` : null;
              ...
              <div className="p-8">
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        <div>
                          ...
                          <PriceDisplay
                            ethAmount={courseData.priceInEth}
                            className="text-lg"
                          />
                        </div>
                        <div>
                        ...
                        >
                          ...
                          <div className="text-center space-y-2">
                            ...
                            <div className="text-sm font-medium">
                              <PriceDisplay
                                ethAmount={(
                                  parseFloat(courseData.priceInEth) *
                ...
                <div className="mt-8 p-6 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-400">
                        {selectedOption.label} access to &ldquo;{course.title}
                        &rdquo;
                      </p>
                    </div>
                  ...
                  <Button
                    ...
                    className="w-full h-14 text-lg bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-lg text-white disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isEnrolling ? (
                      <>
